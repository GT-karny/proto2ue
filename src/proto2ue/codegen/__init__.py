...
@dataclass(frozen=True, slots=True)
class _HeaderIncludePlan:
    engine_includes: List[str]
    dependency_includes: List[str]
    generated_include: Optional[str]
...
class DefaultTemplateRenderer:
    ...
    def _render_header(self, header_name: str, ue_file: UEProtoFile) -> str:
        ...
        lines.append("")
        lines.extend(self._format_header_includes(header_name, ue_file))
        lines.append("")
    ...
    def _format_header_includes(self, header_name: str, ue_file: UEProtoFile) -> List[str]:
        plan = self._collect_header_include_plan(header_name, ue_file)
        lines: List[str] = []
        for include in plan.engine_includes:
            lines.append(f'#include "{include}"')
        if plan.dependency_includes:
            if lines:
                lines.append("")
            for include in plan.dependency_includes:
                lines.append(f'#include "{include}"')
        if plan.generated_include:
            if lines:
                lines.append("")
            lines.append(f'#include "{plan.generated_include}"')
        return lines

    def _collect_header_include_plan(
        self, header_name: str, ue_file: UEProtoFile
    ) -> _HeaderIncludePlan:
        engine_includes = {"CoreMinimal.h"}
        dependency_protos: set[str] = set()

        uses_array = False
        uses_map = False

        for message in self._iter_messages(ue_file.messages):
            for field in message.fields:
                if self._field_uses_array(field):
                    uses_array = True
                if self._field_uses_map(field):
                    uses_map = True
                dependency_protos.update(self._field_dependency_protos(field))

        for wrapper in ue_file.optional_wrappers:
            if self._type_uses_array(wrapper.base_type):
                uses_array = True
            if self._type_uses_map(wrapper.base_type):
                uses_map = True

        if uses_array:
            engine_includes.add("Containers/Array.h")
        if uses_map:
            engine_includes.add("Containers/Map.h")

        local_proto = ue_file.source.name if ue_file.source is not None else None

        dependency_includes: List[str] = []
        for proto_name in sorted(proto for proto in dependency_protos if proto):
            if proto_name == local_proto:
                continue
            dependency_includes.append(self._header_name_for_proto(proto_name))

        engine_list = sorted(engine_includes, key=self._include_sort_key)
        generated_include = self._generated_header_include(header_name)
        return _HeaderIncludePlan(
            engine_includes=engine_list,
            dependency_includes=dependency_includes,
            generated_include=generated_include,
        )

    def _iter_messages(self, messages: Iterable[UEMessage]) -> Iterable[UEMessage]:
        for message in messages:
            yield message
            yield from self._iter_messages(message.nested_messages)

    def _field_dependency_protos(self, field: UEField) -> Iterable[str]:
        for proto in (
            field.dependency_proto,
            field.map_key_dependency_proto,
            field.map_value_dependency_proto,
        ):
            if proto:
                yield proto

    def _field_uses_array(self, field: UEField) -> bool:
        return any(
            self._type_uses_array(type_name)
            for type_name in (
                field.base_type,
                field.ue_type,
                field.map_key_type,
    ...
    def _field_uses_map(self, field: UEField) -> bool:
        return any(
            self._type_uses_map(type_name)
            for type_name in (
                field.base_type,
                field.ue_type,
            )
    ...
    def _type_uses_array(self, type_name: Optional[str]) -> bool:
        return bool(type_name) and "TArray<" in type_name

    def _type_uses_map(self, type_name: Optional[str]) -> bool:
        return bool(type_name) and "TMap<" in type_name

    def _include_sort_key(self, include: str) -> tuple[int, str]:
        return (0 if include == "CoreMinimal.h" else 1, include)

    def _header_name_for_proto(self, proto_name: str) -> str:
        base = proto_name[:-6] if proto_name.endswith(".proto") else proto_name
        return f"{base}{self._header_suffix}"
